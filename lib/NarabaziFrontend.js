    ...
    reqwest ({ // request all links from the data controller
      ...
      success: function(data){
		...
		for (var key in myObj)
		{
			...
			{
				case "youtube":
				$("#right_panel").append('<iframe width="420" height="315" src='+myObj[key].link+' frameborder="0" allowfullscreen></iframe>');
				Narabazi.message('youtube links'+myObj[key].link, 1);
				break;
				case "image":
				$("#right_panel").append('<img src='+myObj[key].link+' />');
				Narabazi.message('flickr links', 1);
				break;
				case "text":
				$("#right_panel").append(myObj[key].link);
				Narabazi.message('flickr links', 1);
				break;
				default:
