...
var Narabazi = {
  ...
  url : function() { // declare our endpoints
    ...
    return {
      ...
      linkTypes   :    baseUrl + 'link/types'// get all the link types
    };
  },
    ...
    $(this.modules).each(function(k, filename) {
      $.getScript(filename)
        .done(function(script, textStatus) {
          Narabazi.message('Narabazi: module loading: ' + textStatus + ' (' + filename + ') (' + (loadedModules + 1) + '/' + Narabazi.modules.length + ')', 1);
  ...
  */
  getMap: function() {
    if (Narabazi.options.map == undefined) {
      Narabazi.message('Narabazi: The map object has not been set.', 0);
    }
  ...
  */
  markerClickedAction: function(e) {
    Narabazi.message('Narabazi: Marker location clicked: ' + e, 1);
    Narabazi.core("showMarkerInfo").publish(e);
  },  
