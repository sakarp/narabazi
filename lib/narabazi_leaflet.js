      ...
      success: function(data){
        ...
        $.link.linkTemplate("#link_list", data)
          ...
          .on("click", "#button_link_add", function() {
            ...
            $.link.addLinkTemplate( "#link_add", linkDetails, contextHelpers );
            ...
            $("#submitAddLink").on( "click", function() {
              ...
              ({
                ...
                method: 'post',
                // send the click events data
                data: { type: linkDetails.selectedType, 
                         title: linkDetails.title, 
                         link: linkDetails.link, q: true },
                success: function (response)
                { 
                  Narabazi.message('Link added with id: ' + response, 1);
                  // now clear and reset the form
                  // clear links and reset
                  $('#selType').val( $('#selType').prop('defaultSelected') );
                }
              });
...
//    Narabazi.displaying
  ...
  var contextHelpers = {
			...
			selectedType: function(selectedType) {
			  console.log('oh ho');
				return (selectedType!=="none") ? types[selectedType].name : "";
			},
			and: function(a, b) {
				return !!a && !!b;
			}
