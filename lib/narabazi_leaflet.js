...
var Narabazi =
{
  url : // declare our endpoints
  {
    ...
    // where to send our new nodes to modify them
    locationAdd : 'http://localhost/discontented/web/app_dev.php/add',
    locationRemove : 'http://localhost/discontented/web/app_dev.php/remove/',
  },
  ...
  */
  ...
  {
    Narabazi.message('Location clicked: ' + link, 2);
    // hide the showing field if there is one
    $(Narabazi.displaying).hide();
    // toggle the field
    place = Narabazi.displaying!='#leftcolumn' ? '#leftcolumn' : '#rightcolumn';
    // set the new showing field
    Narabazi.displaying = place;
    // and show
    $(Narabazi.displaying).show();
    
    // get a delete link, then attach it to the map
    Narabazi.getDeleteLink(link).appendTo($(Narabazi.displaying).first());
  },
